...
function filterTree(tree, filterFn) {
	return tree
		.map((node) => {
			const children = node.children ? filterTree(node.children, filterFn) : undefined
			return {
				name: node.name,
				id: node.id,
				children: children,
...
const filteredTreeData = filterTree(treeData, (node) => node.id === "1" || node.id === "5" || node.id === "55")
